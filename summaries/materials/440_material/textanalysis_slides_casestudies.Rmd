...
- We see that "health" and "care" are the two most common co-occurring words within a single tweet, and "country" and "people" also commonly co-occur. 
...
- In this case, a corpus can be represented as a document-term matrix. Each row is a document, each column is a word, and the $ij$-th entry is the number of times word $j$ appears in document $i$. 
- These matrices are extremely large but also extremely sparse. Most entries are 0. Special tools have been made to represent them in a memory-saving manner. 
...
- The classification task we will focus on is recovering political party from the paragraphs of a speech. 
- We have `r nrow(sou_data_dfm)` documents but `r ncol(sou_data_dfm)` unique words! Logistic regression can only work with fewer features than observations ($p < n$). 
- Need some way to reduce the dimensions 

## Logistic regression - sentiment analysis

One way we discussed before is mapping each word to a "sentiment". We can reduce the columns of the dfm to counts of the sentiment words of each type. For each row (paragraph), sum all the counts of words for each sentiment. The output is a count matrix with the $ij$ entry showing the number of words in row $i$ with sentiment $j$. 

```{r dfm_to_dsm,cache=T,echo=F}
sou_data_dfm <- 
  sou_data_dfm %>% 
...
```{r dsm_logit,cache=F}
logit_sentiment <- glm(party == "Democrat" ~ . -n,
...
```{r paragraph_logit_results,cache=F, echo = F}
...
- What is a multinomial distribution? Just like a Binomial, there are $n$ independent trials, but each trial can have $k$ unique, discrete outcomes. Say you ask $n=10$ people what their favorite color is and tabulate the responses. The responses plausibly have a multinomial distribution. 
- So if  $\boldsymbol{w} | \boldsymbol{\theta} \sim \text{Multinomial}(n,\boldsymbol{\theta})$, then the pmf is $p(\boldsymbol{w}) = \dfrac{n!}{w_1! \cdots w_k!}\theta_1^{w_1}\cdots \theta_k^{w_k}$, where $\sum_i^k w_i = n$ and $\sum_i^k \theta_i = 1$.

## Generative Models: Conjugate priors
- Of course, $\theta_r$ and $\theta_d$ are unknown and we would like to estimate them from the data. Sounds like posterior inference!
- Luckily there is a nice conjugate prior for multinomial data! 
- Dirichlet distribution: $\theta_p \sim \text{Dirichlet}(\alpha)$. 

## Dirichlet Distribution
- The Dirichlet distribution is a multivariate Beta distribution, and is conjugate with the multinomial likelihood, similar to the Beta and Binomial conjugacy.
- The Dirichlet is just like a Beta distribution, but with $k$ parameters rather than two. $\boldsymbol{\theta} = (\theta_1, \ldots,\theta_k) \sim \text{Dir}(\mathbf{z})$. The pdf is proportional to $p(\boldsymbol{\theta}) \propto \prod_{i=1}^k \theta_i^{z_i-1}$. This implies that $\sum_i^k \theta_i = 1$ and $\theta_i \in (0,1)$. 
- $E[\theta_i] = \frac{z_i}{\sum_i^k z_i}$. As the sum $\sum_i^k z_i$ increases (often called the "concentration" of the Dirichlet), the distribution gets more and more concentrated around the means. In priors, the parameters are often all set to the same value and we write $\theta \sim \text{Dir}(\alpha)$ to imply that $E[\theta_i] = 1/k$ with concentration $k\alpha$. 
...
What we are interested in is $P(\text{Dem}|W_{new},W_d,W_r)$, where $W_{new}$ is the new document, and $W_d$ and $W_r$ are word counts from documents authored by Democrats and Republicans.

From Bayes rule, we know $P(\text{Dem}|W_{new},W_d,W_r) \propto P(W_{new}|\text{Dem},W_d,W_r)P(\text{Dem})$. The prior $P(\text{Dem})$ is easy, we choose it ourselves. But how do we compute the other term? 
...
A Dirichlet-Multinomial distribution is the result of $n$ independent, discrete-outcomed trials (same as in the multinomial) where the success probability is drawn from a Dirichlet distribution with specified parameters. This is the same form as the posterior-predictive for our example. Thus, 
...
The results of LDA applied to the State of the Union speeches are shown below. We identified 5 themes/topics. The 10 most common words in each topic are shown below. 

```{r lda_results,echo=F}
lda_speech <- sou_dfm_by_speech %>% 
  dfm_remove("america|people|congress|tonight|govern|time",valuetype = "regex") %>% 
  {.[ntoken(.)>0,]} %>% 
  topicmodels::LDA(k=5,control = list(seed = 0901))
```

```{r top_words}
topicmodels::terms(lda_speech,10)
```

## Topic Models - Motivation

The proportion of times each president spoke about each toipc is shown below

```{r pres_dists,echo=F}
topicmodels::posterior(lda_speech)$topics %>% round(2) %>% cbind(sou_dfm_by_speech@docvars %>% select(pres,year)) %>% 
  group_by(pres) %>% 
  mutate(year = min(as.numeric(year))) %>% 
  summarise_all(mean) %>% 
  arrange(year) %>% 
  select(-year) %>% 
  rename_if(is.numeric,function(t) str_c("Topic ",t)) %>% 
  mutate_if(is.numeric,function(v) round(v,2)) 
```

## Topic Models - LDA

LDA, the Latent Dirichlet Allocation, is the most common topic model. In LDA, each document is modeled as a mixture of $K$ substantive topics. $\theta_d$ gives the proportion of words in document $d$ about each topic, i.e. if $\theta_{d} = <.25,.50,.25>$, then one quarter of the words are from topic 1, one half from topic 2, and one quarter from topic 3. 

A topic is a probability distribution over words. Given that a word is from topic $k$, the probability of drawing a specific word $w$ is given by $\beta_{kw}$. Each topic gives a different probability for drawing that same word $w$. 

Independent Dir($\phi$) priors are placed on each $\beta_k$ and Dir($\alpha$) priors on $\theta_d$. 

## Topic Models - LDA 

Most topic modeling packages in R have built in estimation methods that are very fast. 

Results are often characterized not by inference on a specific parameter, but by visualizing topics and comparing documents by what toipcs they discus. 

Sometimes the document-topic proportions are passed to a second algorithm as a dimension reduction tool to learn something about the documents (similar to how we used sentiment scores for logistic regression). 
