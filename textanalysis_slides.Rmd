...
- Here we are plotting term frequency $n/\text{total}$ for each candidate, that is, the number of times a word is used by a candidate divided by the total number of words used by that candidate
...
```{r cand-freq, warning = F}
ggplot(frequency %>% 
         gather(candidate, word), aes(x = word, fill = candidate))+
  geom_histogram(show.legend=F)+
  xlim(NA, .009)+
  xlab("term frequency")+
  facet_wrap(~candidate, ncol = 2, scales = "free_y")
```

## `bind_tf_idf()` function
- The `bind_df_idf()` function takes a tidy text dataset as input and returns a dataframe with tf, idf, and tf-idf calculated

```{r tf-idf}
candidate_words <- tidy_tweets %>%
  select(screen_name, word) %>%
  count(screen_name, word, sort= T) %>%
  bind_tf_idf(word, screen_name, n)
candidate_words %>% slice(1:4)
```
...
```
- Here, the most distinctive words are much more personal or characteristic of the candidate's platform
  - Biden talks a lot about himself, whereas Warren talks more about political buzzwords
...
## Make and filter to remove custom stop_words
```{r http-stop-words}
...
```{r create-sections, eval = T, echo= F}
...
- We see that "health" and "care" are the two mostcommon co-occurring words within a single tweet, followed by "country" and "people"
- However, you may recall that those words are also most common indiviudal unigrams
...
- A correlation close to 0 means that the two words are not likely to co-occur, and a correlation close to 1 means that the two are both likely to occur or not occur
...
- Notice that "health" and "care" are highly correlated, but we do not see "people" and "country" here

## Pairiwse correlation cont.
- When we looked at pairwise counts, "people" and "country" occurred frequently in a single tweet
- But how correlated are they?
```{r pairwise-cor-low}
word_cors  %>%
  filter(item1 == "people" & item2 == "country")
```
- So even though "people" and "country" co-occur very often within a single tweet, they are not very associated

## Graph of highly correlated words
- Lastly, we can visualize the correlations and clusters of highly-corelated word pairs
